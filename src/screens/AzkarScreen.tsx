...
export default function AzkarScreen() {
  ...
  const azkarCategories = [
    { title: 'Morning Adhkar', time: 'After Fajr', icon: 'partly-sunny', color: '#fbbf24', count: 27 },
    { title: 'Evening Adhkar', time: 'After Asr', icon: 'moon', color: '#93c5fd', count: 27 },
    { title: 'After Prayer', time: 'Post-Fard', icon: 'time', color: '#6ee7b7', count: 12 },
    { title: 'Before Sleep', time: 'Night', icon: 'bed', color: '#c4b5fd', count: 15 },
    { title: 'Contextual Duas', time: 'Daily Life', icon: 'compass', color: '#fca5a5', count: 40 },
  ];
    ...
    <View className="flex-1 bg-emerald-950">
      ...
      <View className="px-6 pt-16 pb-4">
        <View className="flex-row justify-between items-center mb-2">
          <Text className="text-amber-400 text-3xl font-extrabold tracking-tight">Dua & Azkar</Text>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 24, paddingBottom: 100 }}>
            ...
            <View className="p-6">
              <View className="flex-row justify-between items-center mb-4">
                <View className="flex-row items-center bg-amber-500/20 px-3 py-1.5 rounded-full border border-amber-500/30">
                  ...
                  <Text className="text-amber-400 text-xs font-bold uppercase tracking-widest">Digital Tasbih</Text>
                </View>
                <Ionicons name="arrow-forward" size={20} color="#fbbf24" />
              </View>
              
              <Text className="text-white text-2xl font-bold tracking-wide mb-2">Smart Counter</Text>
              <Text className="text-emerald-100/80 text-sm leading-relaxed mb-6 font-medium">
                Keep track of your daily Dhikr with haptic feedback and target goals.
              </Text>
              ...
              <Animated.View style={{ transform: [{ scale: pulseAnim }] }} className="rounded-full overflow-hidden border-2 border-emerald-600/50">
                ...
                <View className="py-3 items-center flex-row justify-center">
                  <Text className="text-emerald-50 font-bold tracking-wide">Open Tasbih</Text>
                </View>
              </Animated.View>
            </View>
          </View>
        ...
        >
          ...
          <View className="p-5 flex-row items-center">
            ...
            <View className="flex-1">
              <Text className="text-emerald-50 font-bold text-lg mb-0.5">Asma-ul-Husna</Text>
              <Text className="text-emerald-300/80 text-xs font-medium">Explore the 99 Beautiful Names</Text>
            </View>
        ...
        {/* Categories Section */}
        <Text className="text-emerald-50 text-xl font-bold tracking-wide mb-4">Categories</Text>
          ...
          {azkarCategories.map((category, idx) => (
            <Animated.View key={idx} style={{ opacity: listOpacities[idx], transform: [{ translateY: listAnims[idx] }] }}>
              <TouchableOpacity 
                ...
                onPress={() => {
                  if (category.title === 'Contextual Duas') {
                ...
                <View className="p-5 flex-row items-center">
                  ...
                  <View className="items-end">
                    <View className="bg-teal-900/60 px-2 py-1 rounded-md border border-teal-700/50 mb-1">
                      <Text className="text-teal-200 text-xs font-bold">{category.count} Duas</Text>
                    </View>
